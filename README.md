...
为保护版权内容，书籍栏目实现了客户端权限控制机制：

- **密码保护**：访问书籍页面需要输入密码（默认：`yiwangMYF`）
- **本地存储**：验证状态保存7天（使用localStorage）
- **URL令牌**：支持通过URL参数快速授权（`?access=yiwangMYF`）
- **默认权限**：本人（yiwangMYF）默认具有访问权限
- **PDF保护**：PDF文件已重命名，增加直接访问难度

**实现方式**：
- `book.md`：书籍列表页面，包含密码验证表单
- `books/redis-preview.md`：PDF查看页面，自动检查验证状态
- 客户端JavaScript验证，不依赖服务器端会话

**修改密码**：
编辑 `book.md` 和 `books/redis-preview.md` 中的 `BOOK_AUTH_PASSWORD` 变量值。

**安全说明**：
- 此方案为客户端基本保护，防止普通用户意外访问
- 技术用户仍可能查看源代码获取密码
- 如需更强保护，请考虑服务器端验证或内容加密
